...
def main():
    ...
    st.subheader(tool.get_company_name(ticker))

    col1, col2, col3, col4 = st.columns(4)
    low, high, yoy, last_year_price = tool.get_yoy_price_data(ticker)
    with col1:
        st.metric("Last Price", f"${tool.get_last_price(ticker):.2f}")
    with col2:
        st.metric("Price Difference (YoY)", f"${(tool.get_last_price(ticker)-last_year_price):.2f}", f"{yoy:+.2f}%")
    with col3:
    ...
    callbelow_df, callabove_df = tool.get_relevant_call_options(ticker, range)
    putbelow_df, putabove_df = tool.get_relevant_put_options(ticker, range)
